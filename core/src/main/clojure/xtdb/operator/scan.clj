...
(defn ->temporal-min-max-range [^IIndirectRelation params, {^TransactionInstant basis-tx :tx}, {:keys [for-valid-time for-system-time]}, selects]
  (let [min-range (temporal/->min-range)
    ...
    (letfn [(apply-bound [f col-name ^long time-μs]
      ...
      (let [ ;; HACK, either in normal form or in datalog form, breaks if mixing conventions
            col-types (if (and (not-empty selects) (str/includes? (ffirst selects) "/"))
                        (update-keys temporal/temporal-col-types (comp symbol util/normal-form-str->datalog-form-str))
                        (update-keys temporal/temporal-col-types symbol))
